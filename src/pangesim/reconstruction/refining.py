...
class SequentialEdgeRefinement(RefinementStrategy):
    ...
    __slots__ = ("id_pool")
    def __init__(self, id_pool:GlobalGenomePool) -> None:
        """Needs a GlobalGenomePool class for genome id creation."""
        self.id_pool = id_pool

    def build_sorted_residuals(self, matrix:AdjacencyMatrix, pan:Pangenome) -> List:
        """Sorts the AdjacencyMatrix's tuples in decreasing order.

        Args:
            matrix: Input adjacency matrix in the form ((node,node),weight)
            pan: Base pangenome object.
        """
        r = build_residuals(pan, matrix)
        residuals = [ (element, r[element]) for element in r if r[element] > 0]
    ...
    def new_genome_with_edge(self, pan:Pangenome, edge:Tuple) -> Genome:
        ...
        """
        new_id = self.id_pool.get_next_id()
        new_genome = Genome(genome_id=new_id)
        new_genome.add_edge(edge)
        return new_genome
    ...
    def refine(
        ...
        r = self.build_sorted_residuals(source, target)
        iters = 1
