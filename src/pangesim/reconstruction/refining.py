...
from pangesim.reconstruction.utils import pan_cost
...
class CostMinRefinement(RefinementStrategy):
    """Refines a given pangenome using scoring function.

    The optimization goal is to look for a minimum cost.
    """
    ...
    def fix_under_edge(
        self, pan: Pangenome, adj: AdjacencyMatrix, edge: Tuple[int, int], cost: float
    ) -> None:
        """Fix an under-represented edge.
        ...
        Args:
           ...
           adj: Input weighted adjacency graph.
           edge: Edge to be corrected.
           cost: The current score of the Pangenome.
        ...
        if slack:
            ...
            new_pan.replace_genome(best_genome._genome_id, best_genome)
            best_cost = pan_cost(new_pan, adj, self.params["alpha"], self.params["gamma"])
            ...
            # Cost logic for generating an entirely new genome
            g = self.new_genome_with_edge(new_pan, edge)
            new_pan.add_genome(g)
            current_cost = pan_cost(new_pan, adj, self.params["alpha"], self.params["gamma"])
            # If adding an entire genome hurts the score, roll back to original
    ...
    def refine(
        ...
        callbacks = callbacks or []
        residuals: AdjacencyMatrix = build_residuals(target=target, source=source)
        current_cost = pan_cost(target, source, self.params["alpha"], self.params["gamma"])
        converged = False
        iters = 1
        pangenome = target.copy()
        ...
        while not converged:
            ...
            if worst_residual > 0:
                # Fix under-represented edge
                pangenome = self.fix_under_edge(pangenome, source, worst_edge, current_cost)
            elif worst_residual < 0:
                # Fix over-represented edge
                pangenome = self.fix_over_edge(pangenome, source, worst_edge, current_cost)
            else:
                pass

            residuals = build_residuals(target=pangenome, source=source)
            current_cost = pan_cost(pangenome, source, self.params["alpha"], self.params["gamma"])
